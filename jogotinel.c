...
bool IsRandomPosOnSnake(Vector2 pos, const deque<Vector2>& snakeBody1, const deque<Vector2>& snakeBody2)
{
    ...
    {
        for (int j = 0; j < snakeBody2.size(); j++)
        {
            if (Vector2Equals(snakeBody1[i], pos) && Vector2Equals(snakeBody2[i], pos)) return true; 
        }
    }
    return false;
}
...
{
public:
    Snake snake1 = Snake(1);
    Snake snake2 = Snake(2);
    ...
    void CheckIfAte(){
        ...
        if(Vector2Equals(snake1.body[0], food.position)) {
            food.position = food.GenerateRandomPos(snake1.body);
    ...
    void CheckCollisionWithBody() {
        for (int i = 1; i < snake1.body.size(); i++) {
            if (Vector2Equals(snake1.body[0], snake1.body[i])) {
                GameOver();
            }
        }
        for (int i = 1; i < snake2.body.size(); i++) {
            if (Vector2Equals(snake2.body[0], snake2.body[i])) {
                GameOver();
            }
        }
    }
